...
use crate::{
    database::tasks::{self, Model as UserModel},
    utilities::app_error::AppError,
};

use super::{
    create_task_extractor::ValidateCreateTask, ResponseDataTask, ResponseTask,
};
...
pub async fn create_task(
    ...
    let task = new_task
        ...
        .try_into_model()
        .map_err(|error| {
            eprintln!("Error converting task to model: {:?}", error);
            AppError::new(
                StatusCode::INTERNAL_SERVER_ERROR,
                "Error creating task",
            )
